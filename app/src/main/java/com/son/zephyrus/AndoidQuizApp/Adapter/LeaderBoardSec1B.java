...
public class LeaderBoardSec1B extends RecyclerView.Adapter<LeaderBoardSec1B.ViewHolderUsers> {

    private List<UserModel> mUsers;
    private Context mContext;
    ArrayList<Map.Entry<String, Integer>> list;

    public LeaderBoardSec1B(Context context, List<UserModel> fireUsers) {
        mUsers = fireUsers;
        mContext = context;
    ...
    public ViewHolderUsers onCreateViewHolder(ViewGroup parent, int viewType) {
        if(viewType == 1){
            return new LeaderBoardSec1B.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row_gold, parent, false));
        }
        if(viewType == 2){
            return new LeaderBoardSec1B.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row_silver, parent, false));
        }
        if(viewType == 3){
            return new LeaderBoardSec1B.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row_bronze, parent, false));
        }
        else {
            return new LeaderBoardSec1B.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row, parent, false));
        }
    }
