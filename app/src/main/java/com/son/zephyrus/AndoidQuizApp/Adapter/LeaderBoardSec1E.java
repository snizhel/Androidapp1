...
public class LeaderBoardSec1E  extends RecyclerView.Adapter<LeaderBoardSec1E.ViewHolderUsers> {
    ...
    @Override
    public LeaderBoardSec1E.ViewHolderUsers onCreateViewHolder(ViewGroup parent, int viewType) {
        if(viewType == 1){
            return new LeaderBoardSec1E.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row_gold, parent, false));
        }
        if(viewType == 2){
            return new LeaderBoardSec1E.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row_silver, parent, false));
        }
        if(viewType == 3){
            return new LeaderBoardSec1E.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row_bronze, parent, false));
        }
        else {
            return new LeaderBoardSec1E.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row, parent, false));
        }
    }
    ...
    @Override
    public void onBindViewHolder(LeaderBoardSec1E.ViewHolderUsers holder, int position) {
        sortingScore();
